                ...
                <div className="mt-6">
                  ...
                  <div className="actions-grid mt-6 w-full">
                    {result.compliant ? (
                      ...
                      <>
                        <Button variant="ghost" className="px-6 py-4 w-full" onClick={() => { setResult(null); setDims(null); }}>VOLVER A MEDIR</Button>
                        ...
                        <Button variant="primary" className="px-6 py-4 w-full" onClick={() => nav("/payment", { state: { result } })}>{strings[kiosk.lang].goToPayment}</Button>
                      </>
                    )}
                  </div>
                </div>
...
function Payment({ kiosk }) {
  ...
  return (
    <div className="min-h-screen bg-white">
      {kioskData && <LangSwitch kiosk={{ lang: "es", setLang: () => {} }} />} {/* simple placeholder */}
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        <div className="mb-4">
          <label className="block font-semibold mb-2">{strings[kiosk?.lang || 'es'].method}</label>
          <div className="flex gap-3">
            <Button variant={method === "card" ? "primary" : "outline"} onClick={() => setMethod("card")} >{strings[kiosk?.lang || 'es'].card}</Button>
            <Button variant={method === "qr" ? "primary" : "outline"} onClick={() => setMethod("qr")} >{strings[kiosk?.lang || 'es'].qr}</Button>
          </div>
        </div>

        {method && (
          <div className="flex gap-4">
            <Button variant="accent" onClick={pay}>{strings[kiosk?.lang || 'es'].processPayment}</Button>
            <Button variant="ghost" onClick={() => nav("/scan")} >{strings[kiosk?.lang || 'es'].back}</Button>
          </div>
        )}
