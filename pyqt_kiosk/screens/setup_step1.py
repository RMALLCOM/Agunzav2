...
class PantallaSetupPaso1(QtWidgets.QWidget):
    def __init__(self, app):
        ...
        grp = QtWidgets.QGroupBox("Idioma")
        hl = QtWidgets.QHBoxLayout(grp)
        self.rbEs = QtWidgets.QRadioButton("ES"); self.rbEn = QtWidgets.QRadioButton("EN")
        self.rbEs.setChecked(True); hl.addWidget(self.rbEs); hl.addWidget(self.rbEn)

        self.country = QtWidgets.QLineEdit("CL")
        self.tz = QtWidgets.QLineEdit("America/Santiago")
        form = QtWidgets.QFormLayout(); form.addRow("País", self.country); form.addRow("Zona horaria", self.tz)

        cv.addWidget(grp); cv.addLayout(form)
